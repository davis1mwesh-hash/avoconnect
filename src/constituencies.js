// Full list of Kenya's 290 constituencies grouped by county
// Used in Signup for the searchable constituency dropdown

export const CONSTITUENCIES_BY_COUNTY = {
...
// Flat list of all constituencies with their county, for search
export const ALL_CONSTITUENCIES = Object.entries(CONSTITUENCIES_BY_COUNTY).flatMap(
  ([county, constituencies]) => constituencies.map(name => ({ name, county }))
);

// Get constituencies for a specific county (used when county is selected first)
export function getConstituenciesForCounty(county) {
